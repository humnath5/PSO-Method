...
void pso_optimization(fptr fun, int np , int nd, int ni,double* lb, double* ub, double* value, double* gbest, int ni_stop, double tol){
 ...
 double w = RandomDouble(0.4,1.0) , c1 = RandomDouble(1.4,2.0), c2 = RandomDouble(1.4,2.0);
/*
...
/*Assign xpos and xvel a random number to begin optimization*/
...
*/
/*
void multi_pso_local_optimizer(fptr fun, fptr_1d  f,int ig, int np, int nd, int ni, int maxRun, int ntries, int* type, double* lb, double* ub, double* value, double* gbest, int ni_stop, double tol)
{    int i;
     double alpha = 1;
    for( i = 0 ; i < maxRun ; i++) {

       pso_optimization(fun, np , nd, ni, lb, ub, value, gbest, ni_stop,tol);
       local_optimizer(fun, f ,ig, nd, ntries, lb, ub, value, gbest, ni_stop, tol);
